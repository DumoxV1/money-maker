...
def run():
    ...
    assert len(niches) == 29, f"expected 29 niches, got {len(niches)}"
    for n in niches:
    ...
    assert "ai-api-margin-reconciler" in listing
    assert "ai-microsaas-complaint-briefs" in listing
    assert "ai-api-marketplace-launch-ops" in listing
